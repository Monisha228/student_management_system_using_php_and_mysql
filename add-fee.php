...
<h3 class="i-name">Fees</h3>

<?php
		if(isset($_POST["submit"]))
		{
			$edate=$_POST["da"].'-'.$_POST["mo"].'-'.$_POST["ye"];
			$sq="insert into fees(FNAME,DPRT,YEAR,AMOUNT,DUE) values('{$_POST["fname"]}','{$_POST["dep"]}','{$_POST["year"]}','{$_POST["amnt"]}','$edate')";
			if($db->query($sq))
			{
...
<label>Fees Name</label><br>
				<input type="text" class="input3" name="fname" required>
					<br>
<label>Department</label>
							<select name="dep" required class="input3">
						
								<?php 
									 $s="SELECT distinct(DNAME)  FROM department";
									$re=$db->query($s);
										if($re->num_rows>0)
											{
												...
												{
													echo "<option value='{$r["DNAME"]}'>{$r["DNAME"]}</option>";
												}
											}
								?>
							</select><br>
<label>Year</label>
							<select name="year" required class="input3">
								...
								<?php 
									 $s="SELECT distinct(DYEAR)  FROM department";
									$re=$db->query($s);
										if($re->num_rows>0)
											{
												echo"<option value=''>Select</option>";
												while($r=$re->fetch_assoc())
												{
													echo "<option value='{$r["DYEAR"]}'>{$r["DYEAR"]}</option>";
												}
											}
								?>
							</select><br>
<label>Amount</label><br>
				<input type="text" class="input3" name="amnt" required>
					<br>
...
<select name="ye" class="input8">
    	...
    	<option value="2034">2034</option>
		  ...
		  <button type="submit" class="btn" name="submit">Add Fees</button>
		</form>
	</div>
	<footer>
